...
<div className=" w-full flex flex-col items-start justify-start relative gap-10 max-h-[30rem] overflow-y-auto">
    ...
    {chat?.map((i, k) => (
  <div key={k} className={`mb-4 w-full flex flex-col ${i?.role === 'user' ? ' items-start' : ' items-end '}`}>
    <div className={`flex justify-center gap-2 ${i?.role === 'user' ? ' flex-row' : ' flex-row-reverse  '} items-center w-fit p-2 bg-navtexts rounded-full`}>
      {
    ...
    <p className={`bg-greennav/15 p-3 rounded-lg w-fit max-w-[80%] ${i?.role === 'user' ? 'mr-10' : '   ml-10'}`}>
      {i?.message}
    </p>
  </div>
))}
